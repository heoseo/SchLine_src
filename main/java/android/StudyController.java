...
//////////////////////////////////////////
	...
	/*
	 * 안드로이드에서 사진을 업로드 한후 결과를 받아야 하므로 기존 View를 호출하는 부분에서 JSON데이터를 반환하는 형태로 변경한다.
	 */
...
//		if(req.getFile().equals("")){//null에러방지 처리
				if (mfile.getOriginalFilename().equals("")) {// null에러방지 처리
					...
					result = sqlSession.getMapper(StudyDAOImpl.class).edit_profile(change_nick, info_img, user_id);
					...
					/*
					 * 물리적 경로를 기반으로 File객체를 생성한 후 지정된 디렉토리가 있는지 확인한다. 만약 없다면 mkdir로 생성한다.
					 */
					...
					result = sqlSession.getMapper(StudyDAOImpl.class).edit_profile(change_nick, saveFileName, user_id);
